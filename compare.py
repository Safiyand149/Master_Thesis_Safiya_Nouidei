...
Side-by-side visual comparison:
  • Top row     → Baseline (keyword / Jaccard)
  • Bottom row  → Step1    (graph + CamemBERT embeddings)

Both systems share exactly the same dataset (seed=42).
"""
...
import matplotlib
import torch
# Try each backend in turn and keep the first that works.
...
def display_comparison(query, baseline_ranked, step1_ids, step1_scores,
    ...
    for row, ranked in enumerate([
        ...
        for col, (wid, score) in enumerate(ranked):
            ...
            # Highlight IDs that appear in both systems' top results.
