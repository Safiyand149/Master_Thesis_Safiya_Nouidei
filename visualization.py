...
- Poids: caption->image=1.0, caption_label->caption=0.8,
...
# ---------------------------------------------------------------------------
# Color palette per node type (consistent with the thesis)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# FIXED position of all nodes (stable layout across the 6 frames)
# ---------------------------------------------------------------------------
POS = {
    TEXT:          (0.50, 0.92),
    # query labels (top)
...
def draw_frame(step_title, active_nodes, edges, fname,
    ...
    """
    active_nodes : set of visible nodes (colored by their type)
    ...
    edge_labels  : dict {(u,v): "0.9"} displayed weights
    """
    ...
    # Space at the top for the title, node layout within [0, 0.88]
    ax.set_xlim(0, 1); ax.set_ylim(0, 1.12)
    ax.axis("off")
    ax.set_aspect("equal")  # perfect circles
    ...
    for n in active_nodes:
        ...
        if n in dimmed_nodes:
            ...
            color = COL_CAPLABEL  # la query devient un label-source orange
        else:
...
def generate():
    ...
    # Nodes NOT involved in the path -> greyed out
    on_path = {TEXT, "family", "faces", IMG_A}
    # Keep relevant query labels and IMG_A colored too
    keep_colored = on_path | {"memory", "joy", "childhood"}
    dimmed = set(POS.keys()) - keep_colored - {IMG_B}
    # IMG_B stays red (discarded candidate) but its branches greyed out
    dimmed |= {"scene"}  # scene not on the selected direct path
    draw_frame(
