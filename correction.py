...
def semantic_similarity(a: str, b: str) -> float:
    """Retourne la similarité cosinus normalisée entre deux étiquettes."""
    if not a or not b:
        return 0.0
...
def correct_hierarchy(hierarchy, similarity_threshold=0.45, min_improvement=0.10, max_iterations=3):
    """Corrige une hiérarchie de labels en se basant sur la similarité sémantique.

    L'algorithme essaie de réaffecter un nœud seulement si un parent alternatif
    est clairement meilleur que le parent actuel. Le détachement pur est évité
    afin de ne pas casser les relations explicites déjà présentes dans la
    hiérarchie source.
    """
...
def correct_work_hierarchy(json_input, work_id, similarity_threshold=0.45, parse_similarity_threshold=0.3):
    """Charge une œuvre JSON et corrige les hiérarchies de subjectMatter."""
    try:
