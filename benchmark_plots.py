...
benchmark_plots.py
==================
Standalone, English-language plotting module for the benchmark comparison
between the proposed graph-based method ("Graph") and CLIP.

This module is self-contained: it only needs the numeric results
(dictionaries with avg/std time and memory) and produces publication-quality
figures. It is imported by benchmark.py but can also be run directly with
synthetic data for a quick visual check:

    python benchmark_plots.py        # renders demo figures into ./benchmark_demo/

Design choices
--------------
* Consistent two-colour palette (Graph = blue, CLIP = orange).
* Because CLIP inference is ~100-200x faster, raw side-by-side bars are
  unreadable; the inference-time figure therefore uses a log y-axis, and the
  former "speedup" chart is reframed as an honest "Inference time ratio"
  (how many times slower the graph method is) on a log scale, matching the
  thesis framing of an interpretability/speed trade-off.
* RSS memory deltas are clipped at 0 (OS page-cache effects can make a raw
  delta negative) and the noise is shown via error bars.
"""
...
# ---------------------------------------------------------------------------
# Global, professional matplotlib style
# ---------------------------------------------------------------------------
...
# Palette: the proposed method is "Graph", compared against "CLIP".
COLORS = {
...
def _bar_value_labels(ax, bars, values, fmt="{:.1f}", unit="",
    ...
    """Write a numeric label above each bar (robust to log axes)."""
    y0, y1 = ax.get_ylim()
...
# ---------------------------------------------------------------------------
# Figure 1 — Indexing (training) time
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Figure 2 — Inference time per query (log scale, both methods visible)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Figure 3 — Indexing memory footprint (delta RSS)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Figure 4 — Inference time ratio (graph / CLIP), log scale, honest framing
# ---------------------------------------------------------------------------
    ...
    for q_idx, (query, g_res, c_res) in enumerate(
        ...
        for row, (top_ids, method, res) in enumerate(
            ...
            for col, wid in enumerate(top_ids):
                ...
                # Shared image code path with main.py when available.
