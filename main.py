...
if DEVICE.type == "cuda":
    ...
    try:
        ...
        pass
    print(f"Pas de GPU — fallback CPU ({os.cpu_count()} threads)")
...
try:
    ...
    print("torchvision disponible : extraction de features visuelles activée")
except ImportError:
    USE_IMAGE_FEATURES = False
    print("torchvision non installé : features visuelles désactivées")

# matplotlib backend — try each in turn and keep the first that works.
import matplotlib
...
# French stopwords for caption preprocessing (used during label extraction).
STOPWORDS = {
...
PHASE1_POOL_SIZE = 50   # candidates retrieved from the graph
PHASE1_DISPLAY   = 5    # always show 5 in Phase 1
...
_IMAGE_FEAT_CACHE = {}        # work_id -> np.array (L2-normalized feature vector)
...
def _wid_has_image(work_id: str) -> bool:
    """Fast lookup against the already in-memory index."""
...
def filter_ids_with_images(sorted_ids, all_scores, target_k):
    """Filter and return exactly target_k IDs that have an image."""
    valid_ids = []
...
def _load_img_or_placeholder(ax, work_id: str):
    """Load the image into ax, or show a grey placeholder if it can't be found."""
...
def display_phase1_grid(top_ids: list, all_scores: dict, moods: dict,
    ...
    """Show the n_display best Phase 1 artworks in a 1×N grid."""
    id_to_item = {item["workID"]: item for item in dataset}
    entries    = top_ids[:n_display]
    n          = len(entries)

    if n == 0:
        print("Aucune image à afficher pour la Phase 1.")
        return
...
def display_phase2_single(wid: str, emotion: str, score: float,
    ...
    """Show the single Phase 2 result in its own window."""
    id_to_item = {item["workID"]: item for item in dataset}
    item       = id_to_item.get(wid, {})
    title      = item.get("objectWork", {}).get("titleText", "") or "—"
...
def get_image_feat_model():
    """Load pretrained ResNet50 as a singleton."""
...
def _flush_image_batch(model, wids, tensors, out_dict):
    """Helper: run one CNN batch and store the L2-normalized features."""
...
def extract_image_features_batch(work_ids: list, batch_size: int = 32) -> dict:
    """
    Extract visual features for a list of work_ids in batches.
    Returns a dict {work_id: np.array} for the IDs that have an image.
    """
...
def _get_valence(emotion_text: str) -> int:
    ...
    for key, val in EMOTION_VALENCE.items():
        if low == key:
            return val
    # Not an exact match: fall back to the closest known emotion by embedding.
...
def load_dataset(json_path):
    ...
    for rec in raw:
        ...
        # Skip malformed or composite work IDs.
...
def extract_caption_labels(caption, global_freq=None, doc_freq=None, n_docs=1, common=None):
    """
    Extract the query's labels for gem.py.

    Improvement over the baseline (hard frequency > 50 filter):
    - Tokens are weighted by IDF; only those whose IDF is too low (< 0.3) are
      rejected, which keeps common but informative domain terms (e.g. "paysage",
      "portrait") that the baseline discards.
    - Very short tokens (≤ 2 chars) and stopwords are always excluded.
    - Bigrams are added to capture compound expressions.
    """
    tokens = caption_preprocessing(caption)
    result = []
    seen = set()

    # Filtered single tokens
    for t in tokens:
        if len(t) > 2 and t not in STOPWORDS and t not in seen:
            if common is None or t not in common:
                result.append(t)
                seen.add(t)

    # Bigrams: "jeune garçon", "manteau sombre", "chien noir"
    for i in range(len(tokens) - 1):
        t1, t2 = tokens[i], tokens[i+1]
        if t1 not in STOPWORDS and t2 not in STOPWORDS:
            bigram = f"{t1} {t2}"
...
def extract_captions_moods_batch(dataset, chunk_size=200):
    """
    Extract moods in batches.
    - Items WITH a description: text-based inference via embeddings.
    - Items WITHOUT a description: fall back to "autre".
    """
    if DEVICE.type == "cuda":
        chunk_size = 200
    else:
...
def extract_captions_moods(caption):
    ...
    phrases = [p.strip() for p in re.split(r'[.!?,;]', caption) if len(p.strip()) > 5]

    # Single sentence or short caption: original behaviour.
    if len(phrases) <= 1:
        embeddings = _embed([caption] + EMOTION_LABELS)
        sims = embeddings[0] @ embeddings[1:].T
        return [EMOTION_LABELS[int(np.argmax(sims))]]

    # Encode all the sentences in one batch.
    all_texts = phrases + EMOTION_LABELS
    embeddings = _embed(all_texts)
    phrase_embs = embeddings[:len(phrases)]
    emotion_embs = embeddings[len(phrases):]

    # Dominant emotion per sentence.
    votes = []
    for phrase_emb in phrase_embs:
        sims = phrase_emb @ emotion_embs.T
        votes.append(int(np.argmax(sims)))

    # Majority vote.
    best_emotion_idx = Counter(votes).most_common(1)[0][0]
    return [EMOTION_LABELS[best_emotion_idx]]
...
def train_model(cap_dict, hier_dict):
    ...
    # Co-occurrence of caption labels within the same artwork.
    for labels in cap_dict.values():
        for i, l1 in enumerate(labels):
            for l2 in labels[i + 1:]:
                counts[_p(l1, l2)] += 1

    # Co-occurrence of hierarchy nodes, weighted by their depths.
    ...
    # Links between caption labels and hierarchy labels of the same artwork.
...
def construct_graph(cap_dict, hier_dict, mood_dict, trained, image_feat_dict=None):
    ...
    # All known wids (with or without a description).
    ...
    # Convert edge weights into distances for shortest-path propagation.
...
# Query vectorization and mapping
...
def _map_query_to_graph(G, query_labels, threshold=0.45):
    ...
    if USE_FAISS:
        ...
        for i, ql in enumerate(text_labels):
            ...
            # Guaranteed fallback: if nothing clears the threshold, take the best.
...
def _compute_similarity(G, query_labels, top_k=PHASE1_POOL_SIZE, doc_freq=None, n_docs=1):
    """
    Phase 1 semantic retrieval. Improvements over the baseline:

    1. Real IDF for the mood labels (based on doc_freq) rather than a constant
       0.5 — rarer emotions are more discriminative.
    2. Node-type bonus: caption_label nodes (free text) get a ×1.2 boost,
       hier_label nodes a boost proportional to their depth (more specific term =
       more precise), mood nodes ×1.0.
    3. Propagation cutoff extended to 4.0 (vs 3.0) to reach more peripheral images
       that share close but indirect concepts.
    4. Normalization by sqrt(degree) instead of log(degree): rewards well-
       connected images (label-rich) without crushing them.
    5. Multi-label coverage score: an extra bonus when several query labels
       converge on the same image (a strong relevance signal).
    """
    image_nodes = [n for n, d in G.nodes(data=True) if d.get("type") == "image"]
    mapped      = _map_query_to_graph(G, query_labels)

    # Unified IDF: same formula for every label (no more 0.5 constant).
    ...
    # Per-graph-node-type bonus.
    NODE_TYPE_BOOST = {
        "caption_label": 1.2,
        "hier_label"   : 1.1,   # also modulated by depth below
        "mood"         : 1.0,
    }
    ...
    label_hits  = {img: set() for img in image_nodes}   # for the coverage bonus
    ...
    for ql, gn, cosim in sources:
        ...
        if gn_type == "hier_label":
            type_boost *= (1.0 + 0.1 * depth)   # more specific terms = bonus
    ...
    # Coverage bonus: several labels pointing to the same image.
    for img in scores:
        n_hits = len(label_hits[img])
        if n_hits > 1:
            scores[img] *= (1.0 + 0.15 * math.log1p(n_hits - 1))

    # Normalization: sqrt(degree).
    for img in scores:
        if scores[img] > 0:
...
def dense_retrieval(query, dataset, top_k=20):
    """
    Dense retrieval: compare the embedding of the whole query with the embeddings
    of the dataset's full descriptions.
    """
    # Embedding of the complete query.
    q_emb = _embed([query])[0]

    # Description embeddings (with _embed's implicit cache).
    captions = [item["iconographicInterpretation"] 
                for item in dataset 
    ...
    cap_embs = _embed(captions)
    sims = cap_embs @ q_emb

    top_indices = np.argsort(sims)[::-1][:top_k]
    return [(wids[i], float(sims[i])) for i in top_indices]


def fusion_scores(graph_scores, dense_results, alpha=0.5):
    """
    Fuse the graph and dense retrieval scores.
    alpha=0.5: equal weight between the two approaches.
    """
    dense_scores = {wid: score for wid, score in dense_results}

    # Normalize the graph scores to 0–1.
    max_g = max(graph_scores.values()) if graph_scores else 1
    norm_graph = {wid: s / max_g for wid, s in graph_scores.items()}

    # Normalize the dense scores to 0–1.
    max_d = max(dense_scores.values()) if dense_scores else 1
    norm_dense = {wid: s / max_d for wid, s in dense_scores.items()}

    all_wids = set(norm_graph) | set(norm_dense)
    fused = {
        wid: alpha * norm_graph.get(wid, 0) + (1 - alpha) * norm_dense.get(wid, 0)
        for wid in all_wids
    }
    return fused


def expand_query_semantically(query_labels, graph_nodes, top_k=3):
    """
    For each query label, find the top_k closest graph nodes and add them as
    extra labels with a reduced weight.
    """
    if not query_labels or not graph_nodes:
        return query_labels

    q_embs = _embed(query_labels).astype("float32")
    n_embs = _embed(graph_nodes).astype("float32")

    sim_matrix = q_embs @ n_embs.T

    expanded = list(query_labels)
    seen = set(query_labels)

    for i, ql in enumerate(query_labels):
        top_indices = np.argsort(sim_matrix[i])[::-1][:top_k]
        for j in top_indices:
            candidate = graph_nodes[j]
            score = float(sim_matrix[i][j])
            if score >= 0.6 and candidate not in seen:
...
def phase2_emotion_filter(top_ids, desired_emotion, mood_dict):
    ...
    for i, (wid, emotion) in enumerate(candidates):
        ...
        # Penalize candidates whose valence is opposite to the desired one.
...
if __name__ == "__main__":
    ...
    # cap_labels: only items with a description.
    cap_labels = {
        item["workID"]: [
    ...
    # Visual feature extraction (ResNet50).
    image_feat_dict = {}
    ...
    while True:
        ...
        # Semantic expansion of the labels.
        graph_nodes = list(G.nodes())
        expanded_labels = expand_query_semantically(q_labels, graph_nodes, top_k=3)
        q_labels = expanded_labels

        # Dense retrieval.
        dense_results = dense_retrieval(query, dataset, top_k=PHASE1_POOL_SIZE)

        # Pull a large candidate pool from the graph.
        top_ids_raw, graph_scores = _compute_similarity(
            G, q_labels,
            top_k=PHASE1_POOL_SIZE,   # large pool (50)
            doc_freq=doc_freq,
            n_docs=n_docs,
        )

        # Fuse the graph and dense scores.
        fused_scores = fusion_scores(graph_scores, dense_results, alpha=0.5)

        # Sort by the fused scores.
        sorted_fused = sorted(fused_scores.items(), key=lambda x: x[1], reverse=True)
        top_ids_raw = [wid for wid, _ in sorted_fused[:PHASE1_POOL_SIZE]]

        # Filter to keep exactly PHASE1_DISPLAY IDs that have an image.
        ...
        # Phase 1 grid: always 5 images.
        display_phase1_grid(top_ids, fused_scores, moods, dataset, query,
