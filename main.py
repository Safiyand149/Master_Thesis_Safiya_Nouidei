...
# STOPWORDS en français pour le pré-traitement des captions (extraction de labels)
STOPWORDS = {
...
PHASE1_POOL_SIZE = 50   # candidats récupérés depuis le graphe
PHASE1_DISPLAY   = 5    # toujours afficher 5 en Phase 1
...
def get_image_feat_model():
    """Charge ResNet50 pré-entraîné en singleton."""
...
def extract_caption_labels(caption, global_freq=None, doc_freq=None, n_docs=1, common=None):
    ...
    - Ajout de bigrams pour capturer des expressions composées.
    """
    tokens = caption_preprocessing(caption)
    result = []
    seen = set()
    
    # Tokens simples filtrés
    for t in tokens:
...
def extract_captions_moods_batch(dataset, chunk_size=200):
    ...
    - Items SANS description  : fallback "autre".
    """
    if DEVICE.type == "cuda":
        chunk_size = 200
    else:
...
# Vectorisation et mapping de la requête
...
def dense_retrieval(query, dataset, top_k=20):
    """
    Recherche dense : compare l'embedding de la requête entière
    avec les embeddings des descriptions complètes du dataset.
    """
    # Embedding de la requête complète
    q_emb = _embed([query])[0]
    
    # Embeddings des descriptions (avec cache implicite de _embed)
...
def fusion_scores(graph_scores, dense_results, alpha=0.5):
    """
    Fusionne les scores du graphe et de la recherche dense.
    alpha=0.5 : poids égal entre les deux approches.
    """
    dense_scores = {wid: score for wid, score in dense_results}
    
    # Normaliser les scores graphe entre 0 et 1
    max_g = max(graph_scores.values()) if graph_scores else 1
    norm_graph = {wid: s / max_g for wid, s in graph_scores.items()}
    
    # Normaliser les scores dense entre 0 et 1
...
def expand_query_semantically(query_labels, graph_nodes, top_k=3):
    """
    Pour chaque label de la requête, trouve les top_k nœuds du graphe
    les plus proches et les ajoute comme labels supplémentaires avec
    un poids réduit.
    """
...
if __name__ == "__main__":
    ...
    # Pré-construction de l'index images 
    ...
    # Items SANS description mais ayant une image 
    ...
    while True:
        ...
        top_ids_raw, graph_scores = _compute_similarity(
            G, q_labels,
            top_k=PHASE1_POOL_SIZE,   # pool large (50)
            doc_freq=doc_freq,
            n_docs=n_docs,
        )
        ...
        # Grille Phase 1 : toujours 5 images
