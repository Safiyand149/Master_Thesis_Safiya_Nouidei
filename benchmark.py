...
import main as _main_module          # gives access to the module's globals
...
# --- Image lookup ---
# CLIP must use the EXACT same image index and lookup as the graph method.
# Otherwise the two pipelines disagree on which workIDs have an image (different
# glob order, casing, cached index, etc.) and CLIP's thumbnails come up empty.
# So we alias directly onto main.py's functions rather than maintaining a second,
# separate CLIP image index.
import glob
import re
...
def truncate_for_clip(text: str, char_limit: int = 300) -> str:
    """Roughly 77 CLIP tokens, about 300 characters."""
    return text[:char_limit] + "…" if len(text) > char_limit else text


def infer_emotion_from_query(query: str) -> str:
    # Truncate so the comparison with CLIP is fair.
...
def _invalidate_graph_cache():
    """
    Resets main.py's global _GRAPH_INDEX and _GRAPH_INDEX_HASH, forcing a full
    rebuild of the vector index on the next call to _get_graph_index().

    The cache lives in the module's globals, not in function attributes, which is
    why we modify the imported module's attributes directly.
    """
...
def train_original(dataset, raw):
    ...
    # Truncate descriptions to 300 chars so the comparison with CLIP is fair.
    truncated_dataset = [
    ...
    _get_graph_index(G)  # build and cache the vector index
...
def _run_original_retrieval_only(query: str, ctx: dict):
    ...
    Phase 2 (emotion filter) is excluded — this is used for pure timing.
    """
    # Truncate the query to 300 chars so the comparison with CLIP is fair.
    query = truncate_for_clip(query)
...
def _run_original_timed(query: str, ctx: dict):
    """Full run used as a warm-up; not measured."""
...
def benchmark_original_training(dataset, raw, n_runs: int = 3) -> dict:
    ...
    for the same reason as CLIP: once CamemBERT embeddings are cached, later runs
    allocate far less and the mean would under-report the true footprint.
    """
    times, mems = [], []
    ctx = None
    ...
    for run in range(n_runs):
        ctx = None
        _invalidate_graph_cache()   # FIX 1: force a rebuild
        gc.collect()
        if DEVICE.type == "cuda":
...
def benchmark_original_inference(query: str, ctx: dict, n_runs: int = 5,
    ...
    for _ in range(n_runs):
        ...
        # Phase 2 runs outside the timed block — for the qualitative result only.
        desired_emotion = infer_emotion_from_query(query)
...
def infer_clip(query: str, ctx: dict) -> dict:
    ...
    # metric and for timing parity with the graph method.
    ...
    # similarity is systematically higher than text->image) and the grid ends up
...
def benchmark_clip_training(dataset: list, n_runs: int = 3) -> dict:
    ...
    Why peak and not mean: each run rebuilds a fresh CLIPRetriever, but on runs
    2+ the previous model and the OS page-cache keep the weights resident, so the
    measured delta-RSS collapses toward zero and the *mean* would under-report
    the true footprint (sometimes below model_mem_mb, which is impossible). So we
    (a) explicitly free the previous context before each run so memory is
    comparable, and (b) report the maximum delta across runs, which corresponds
    to the genuine one-off cost of loading + indexing.
    """
    times, mems = [], []
    model_mem_mb = 0.0
    ctx = None
...
def _save_fig(fig, name: str, out_dir: str):
    """Kept for backward compatibility (the qualitative grid still uses it)."""
...
def plot_qualitative_comparison(queries, graph_infer, clip_infer,
    ...
    Image selection uses the SAME function the graph algorithm uses in main.py —
    main.filter_ids_with_images(sorted_ids, scores, target_k) — applied
    identically to both methods. Image drawing uses the SAME routine —
    ...
    # For each query, pick the top_k displayable IDs for each method using the
