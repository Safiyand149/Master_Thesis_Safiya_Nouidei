...
def _add_edge(hierarchy, parent, child):
    # Add a parent -> child link, making sure both nodes exist and there are no duplicates.
    parent = parent.strip()
    child = child.strip()
...
def _parse_group(hierarchy, main_group, inner, similarity_threshold):
    ...
    for token in tokens:
        ...
        if len(parts) == 3:
            ...
            current_parent = b  # most recent explicit parent for the standalones that follow
            ...
            # Standalone: attach to current_parent if similar enough, otherwise to main_group.
            if current_parent:
...
def _add_field_to_hierarchy(hierarchy, field_text, similarity_threshold):
    """Parse one field's raw text (subjectTerms / iconographicTerms / conceptualTerms
    style syntax) and merge the resulting nodes/edges into *hierarchy* in place.

    The RMFAB-style field may contain several groups separated by ";" at the top
    level (outside parentheses), e.g.:
      "groupe A (x ; y : z) ; groupe B (a ; b : c)"

    Each group becomes an independent root. Within each group, depth rules are
    (see _parse_group):
      depth 0 : main_group
    ...
    text = re.sub(r"\s+", " ", field_text).strip()
    if not text:
        return

    # Split on ';' at the top level (outside parentheses) and add each segment.
    # This correctly handles all of:
    #   - a single bare term with no ';' and no '(' (single root node)
    #   - several bare terms separated by ';' with no '(' at all
    #     (e.g. "jésuite ; Contre-Réforme" -> two independent root nodes)
    #   - segments containing "main_group (...)" groups, parsed via _parse_group
    for segment in _split_top_level(text):
        ...
        if "(" in segment:
            ...
            # Bare segment with no parentheses: a top-level node with no children.
            if segment:
...
FIELD_LABELS = {
    ...
    "iconographicTerms": "Iconographic Terms",
    "conceptualTerms": "Conceptual Terms",
}


def creation_hierarchy(subject_matter, similarity_threshold=0.3):
    # Build an INDEPENDENT hierarchy for each of subjectTerms, iconographicTerms,
    # and conceptualTerms — any of which may be absent. Returns a dict keyed by
    # field name:
    #   { "subjectTerms": {...}, "iconographicTerms": {...}, "conceptualTerms": {...} }
    # Only fields actually present (non-empty) in subject_matter appear in the
    # result. Each field's hierarchy is built independently with the same rules
    # (see _add_field_to_hierarchy / _parse_group).

    hierarchies = {}
    subject_matter = subject_matter or {}

    for field in FIELD_LABELS:
        field_text = subject_matter.get(field, "")
        if not field_text:
            continue
        hierarchy = {}
        _add_field_to_hierarchy(hierarchy, field_text, similarity_threshold)
...
def compute_levels(hierarchy, root):
    # Breadth-first assignment of a depth level to every node reachable from root.
    levels = {root: 0}
    queue = [root]
...
def _get_roots(hierarchy):
    """Return all nodes that are not a child of any other node (i.e. the top-level roots)."""
...
# Palette indexed by depth/degree in the hierarchy (0 = root). Cycles if a
# hierarchy is ever deeper than the palette (shouldn't happen given depth <= 3,
# but kept safe via the modulo below).
DEGREE_PALETTE = [
    "#2563EB",  # degree 0 (root)      - blue
    "#F97316",  # degree 1             - orange
    "#16A34A",  # degree 2             - green
    "#DC2626",  # degree 3             - red
    "#9333EA",  # degree 4+            - purple
]


def _compute_all_levels(hierarchy):
    """Compute depth/degree for every node, across every root subtree."""
    levels = {}
    for root in _get_roots(hierarchy):
        levels.update(compute_levels(hierarchy, root))
    return levels
...
def plot_hierarchy(hierarchy, title="Subject Terms Hierarchy", save_path=None):
    ...
    levels = _compute_all_levels(hierarchy)

    # Colour each node by its depth in the hierarchy.
    node_colors = []
    for node in graph.nodes():
        degree = levels.get(node, 0)
        node_colors.append(DEGREE_PALETTE[degree % len(DEGREE_PALETTE)])
    ...
    # Legend mapping colour -> degree (only for degrees actually present),
    # placed outside the plot area so it never overlaps the graph nodes.
    max_degree = max(levels.values()) if levels else 0
    legend_handles = [
        plt.Line2D(
            [0], [0],
            marker="o", color="w",
            markerfacecolor=DEGREE_PALETTE[d % len(DEGREE_PALETTE)],
            markersize=15,
            label=f"Degree {d}",
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: python hierarchy.py <json_input> <work_id>")
        sys.exit(1)
    ...
    print(f"subject_matter keys: {list(subject_matter.keys())}")
    for field in FIELD_LABELS:
        print(f"  {field!r}: {subject_matter.get(field)!r}")

    hierarchies = creation_hierarchy(subject_matter)

    if not hierarchies:
        print("No subjectTerms / iconographicTerms / conceptualTerms found — nothing to plot.")

    for field, hierarchy in hierarchies.items():
        ...
        if hierarchy:
            plot_hierarchy(
                hierarchy,
                title=f"{label} Hierarchy — Work ID: {work_id}",
                save_path=f"{work_id}_{field}.png",
            )
        else:
