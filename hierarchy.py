...
def _add_field_to_hierarchy(hierarchy, field_text, similarity_threshold):
    ...
    # Split on ';' at the top level (outside parentheses) and add each
    # segment. This correctly handles both:
...
def creation_hierarchy(subject_matter, similarity_threshold=0.3):
    # Builds an INDEPENDENT hierarchy for each of subjectTerms,
    # iconographicTerms, and conceptualTerms — any of which may be absent.
    # Returns a dict keyed by field name:
    #   { "subjectTerms": {...}, "iconographicTerms": {...}, "conceptualTerms": {...} }
    # Only fields that are actually present (non-empty) in subject_matter are
    # included in the result. Each field's hierarchy is built independently
    # with the same rules (see _add_field_to_hierarchy / _parse_group).
...
# but kept safe via modulo below).
DEGREE_PALETTE = [
...
def plot_hierarchy(hierarchy, title="Subject Terms Hierarchy", save_path=None):
    ...
    # Legend mapping color -> degree (only for degrees actually present),
